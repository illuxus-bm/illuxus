    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-start px-4 py-10">
      <div className="w-full max-w-md space-y-5">
        <header className="text-center space-y-1">
          ...
          <h1 className="text-2xl font-semibold leading-tight">{eventTitle || "Event check-in"}</h1>
          <p className="text-[13px] text-muted-foreground">Scan your ticket QR code to check yourself in.</p>
        </header>

        <div id={containerId.current} className="w-full aspect-square rounded-xl overflow-hidden border border-border bg-muted/40" />

        <div className="flex gap-2">
          {!scanning ? (
            <Button onClick={start} className="flex-1 gap-2" disabled={busy}>
