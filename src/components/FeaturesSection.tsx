        ...
        {nodes.map((node, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] font-medium text-white/70">{node.label}</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="relative grid h-[220px] grid-cols-5 gap-3 sm:gap-4">
      ...
      <div className="col-span-2 self-end">
        <div className="relative mx-auto w-[120px] rounded-[28px] border border-white/10 bg-[#111114] p-1.5 shadow-[0_18px_50px_-12px_rgba(0,0,0,0.7)]">
          <div className="overflow-hidden rounded-[22px] bg-[#0E0E12]">
            <div className="flex h-[42px] items-center justify-between px-3 text-[8px] text-white/40">
              <span>9:41</span>
              <span className="flex items-center gap-1">●●●</span>
            </div>
            <div className="px-3 pb-3">
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-2.5">
                <div className="text-[8px] uppercase tracking-widest text-white/40">Scanning</div>
                <div className="mt-1 flex aspect-square items-center justify-center rounded-lg bg-black/40">
      ...
      <div className="col-span-3 self-center sm:col-span-2">
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 backdrop-blur-sm">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[10px] font-medium text-white/70">Live attendees</span>
            ...
            {[
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="h-5 w-5 rounded-full bg-gradient-to-br from-indigo-400/40 to-purple-500/30 ring-1 ring-white/10" />
                  <span className="text-[10px] text-white/80">{row.name}</span>
                </div>
                <span className="text-[9px] text-white/40">{row.time}</span>
              </motion.div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="relative h-[220px] w-full">
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-full max-w-[280px] rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full bg-gradient-to-br from-pink-400/40 to-purple-500/30 ring-1 ring-white/10">
              <Bot className="absolute inset-0 m-auto h-4 w-4 text-white/80" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[11px] font-semibold text-white">Maya Patel</span>
              ...
              <div className="text-[10px] text-white/50">Founder · Helios Labs</div>
            </div>
          </div>
          <div className="mt-3 space-y-1.5">
            {[100, 92, 84, 64].map((width, i) => (
              <motion.div
                ...
                className="h-1.5 origin-left rounded-full bg-gradient-to-r from-white/15 to-white/[0.04]"
                style={{ width: `${width}%` }}
              />
            ))}
          </div>
          <div className="mt-3 flex gap-1.5">
            {["Robotics", "GTM", "Hardware"].map((tag) => (
              <span
                key={tag}
                className="rounded-md bg-white/[0.04] px-1.5 py-0.5 text-[9px] text-white/65 ring-1 ring-inset ring-white/[0.06]"
              >
                {tag}
              </span>
            ))}
...
function PreEventVisual() {
  return (
    <div className="relative w-full overflow-hidden rounded-2xl border border-white/10 bg-[#0E0E12] p-5 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.6)]">
      <div className="mb-4 flex items-center justify-between">
        <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/40">
          Ticket tiers
        </span>
        <span className="text-[10px] text-white/40">Auto-sequence on</span>
      </div>
        ...
        {[
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-medium text-white/70">{tier.name}</span>
              <span className="text-[10px] font-semibold text-white">{tier.price}</span>
            </div>
            ...
            <div className="mt-1 text-[9px] text-white/40">{tier.sold}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-5 flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
        ...
        <span className="text-[10px] font-medium text-emerald-300">Scheduled</span>
      </div>
    </div>
  );
}
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 flex items-baseline gap-2">
            ...
            <span className="text-[11px] text-white/50">of 1,200 in</span>
          </div>
        </div>
      ...
      <div className="space-y-2">
        {[
          ...
          return (
            ...
            >
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br ${row.tint} ring-1 ring-inset ring-white/10`}
              >
                <Icon className={`h-3.5 w-3.5 ${row.color}`} />
              </div>
              <span className="flex-1 truncate text-[11px] text-white/80">{row.msg}</span>
  ...
  return (
    ...
    >
      <AnimatedStack>
        ...
        <AnimatedItem>
          <h3
            className="mt-4 text-balance text-3xl font-semibold leading-[1.1] tracking-[-0.03em] text-white sm:text-4xl md:text-[40px]"
        ...
        <AnimatedItem>
          <ul className="mt-6 space-y-2.5">
            {bullets.map((bullet) => (
              <li key={bullet} className="flex items-start gap-3 text-[14px] text-white/75">
                <CheckCircle2
        ...
        {cta && (
          <AnimatedItem>
            <Link
              to={cta.href}
              className="mt-7 inline-flex items-center gap-1.5 text-[13px] font-semibold text-white/90 transition-colors hover:text-white"
            >
              {cta.label}
    ...
    >
      <SiteContainer>
        ...
        <AnimatedStack className="mx-auto mb-16 max-w-3xl text-center md:mb-20">
          {f.eyebrow && (
            <AnimatedItem>
              <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-indigo-300 backdrop-blur-xl">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div className="flex h-full flex-col p-7 sm:p-8">
              ...
              <h3
                className="text-balance text-[22px] font-semibold leading-tight tracking-[-0.025em] text-white sm:text-[28px]"
                style={{ fontFamily: "Poppins, system-ui, sans-serif" }}
              >
          ...
          <BentoCard glowColor="rgba(168, 85, 247, 0.22)">
            <div className="flex h-full flex-col p-7 sm:p-8">
              ...
              <h3
                className="text-balance text-[22px] font-semibold leading-tight tracking-[-0.025em] text-white sm:text-[24px]"
                style={{ fontFamily: "Poppins, system-ui, sans-serif" }}
              >
                Live check-in sync
              </h3>
          ...
          >
            <div className="flex h-full flex-col gap-6 p-7 sm:flex-row sm:items-center sm:p-8">
              <div className="flex-1">
                ...
                <h3
                  className="text-balance text-[22px] font-semibold leading-tight tracking-[-0.025em] text-white sm:text-[26px]"
                  style={{ fontFamily: "Poppins, system-ui, sans-serif" }}
                >
