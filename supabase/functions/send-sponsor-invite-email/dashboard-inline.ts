/**
 * send-sponsor-invite-email — self-contained Dashboard build (SMTP transport).
 *
 * Sends a rich HTML "you've been added as a sponsor" email modelled after
 * `send-ticket-email`: event banner, organiser block (logo + name), event
 * date + venue, and a single CTA into the sponsor portal.
 *
 * Trigger: organiser-side `SponsorManagement` calls this as fire-and-
 * forget after `INSERT INTO sponsors / event_sponsors`.
 *
 * ── Request body (JSON) ────────────────────────────────────────────────────
 *   {
 *     sponsor_id: string,   // uuid — row in public.sponsors
 *     event_id:   string    // uuid — row in public.events
 *   }
 *
 * ── Required Supabase secrets ──────────────────────────────────────────────
 *   SMTP_HOST, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD, SMTP_FROM (optional)
 *   PUBLIC_DOMAIN | VITE_PUBLIC_DOMAIN | VITE_PUBLIC_ORIGIN (optional;
 *   defaults to https://illuxus.com)
 */

import { createClient } from "https://esm.sh/@supabase/supabase-js@2";
import { SMTPClient } from "https://deno.land/x/denomailer@1.6.0/mod.ts";

// ── CORS (inlined; the Dashboard editor doesn't follow ../_shared) ─────────
...
// ── HTML email template (mirrors send-ticket-email's visual language) ──────
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:28px 32px 24px;">
          ...
          <!-- Sponsor portal CTA — the single call-to-action of this email.
               Lands on /sponsor where they can view event details, manage
               their booth, and access the lead list. -->
...
Deno.serve(async (req) => {
  ...
  try {
    const body = await req.json() as { sponsor_id?: string; event_id?: string };
    const sponsor_id = body.sponsor_id;
    const event_id = body.event_id;
    if (!sponsor_id || !event_id) {
      return json({ error: "sponsor_id and event_id are required" }, 400);
    }
