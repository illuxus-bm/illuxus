 ...
 * Gemini via a raw `fetch` (no `@google/genai` npm dependency) and uses the
 * existing `@supabase/supabase-js` esm.sh import.
 *
 ...
 * The Supabase Dashboard's "Edit function" editor and its "deploy from
 * dashboard" bundler only ship the single file you paste — they don't
 * resolve `../_shared/*` relative imports outside the function's own
 * folder. Importing from `../_shared/cors.ts` caused:
 *   Module not found "file:///tmp/.../_shared/cors.ts"
 * at deploy time. Inlining keeps a single canonical `index.ts` that works
 * whether it's deployed via the CLI (`supabase functions deploy ...`) or
 * the Dashboard editor. Same pattern as `send-event-email/dashboard-inline.ts`.
 *
