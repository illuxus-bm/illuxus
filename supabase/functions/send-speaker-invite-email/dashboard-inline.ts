/**
 * send-speaker-invite-email — self-contained Dashboard build (SMTP transport).
 *
 * Sends a rich HTML "you're a speaker" email modelled after
 * `send-ticket-email` and `send-sponsor-invite-email`: event banner,
 * organiser block, date / venue, single CTA into the speaker portal.
 *
 * Two trigger kinds:
 *
 *   kind = "added"     → organiser added the speaker directly through
 *                        SpeakerManagement. Copy: "You're on stage."
 *   kind = "approved"  → organiser approved a speaker application. Copy:
 *                        "Your speaker application is approved."
 *
 * Both share the same template; only the headline pill / subject line
 * differ so the surface stays consistent with the other email surfaces.
 *
 * ── Request body (JSON) ────────────────────────────────────────────────────
 *   {
 *     speaker_id: string,                 // uuid — row in public.speakers
 *     event_id:   string,                 // uuid — row in public.events
 *     kind?:      "added" | "approved"    // default "added"
 *   }
 *
...
function buildCorsHeaders(req: Request): Record<string, string> {
  ...
  if (origin && (allowed.has(origin) || allowed.has(stripTrailingSlash(origin)))) {
...
function copyFor(kind: Kind, orgName: string, speakerName: string): Copy {
  if (kind === "approved") {
    return {
      ...
      subject: (t) => `Your speaker application is approved — ${t}`,
    };
  }
  return {
    ...
    subject: (t) => `You've been added as a speaker for ${t}`,
  };
}
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:28px 32px 24px;">
          ...
          <!-- Speaker portal CTA — single primary action. -->
...
Deno.serve(async (req) => {
  ...
  try {
    const body = await req.json() as { speaker_id?: string; event_id?: string; kind?: Kind };
    const speaker_id = body.speaker_id;
    const event_id = body.event_id;
    const kind: Kind = body.kind === "approved" ? "approved" : "added";
    ...
    if (!smtpConfigured()) return json({ ok: true, delivered: false, note: "SMTP not configured" });
