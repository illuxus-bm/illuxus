...
illuxus ships as an installable Progressive Web App. Once installed it launches
in standalone mode, has its own icon and splash, works offline for already-visited
pages, and self-updates with a non-intrusive prompt.

### Install

- **iOS / iPadOS Safari** — tap the share icon and choose **Add to Home Screen**.
  A passive tip surfaces on first visit and remembers when it's dismissed.
- **Android Chrome / Edge** — tap the install affordance in the address bar, or
  the floating "Install illuxus" pill that appears on first visit.
- **Desktop Chrome / Edge / Brave** — click the install icon in the right side
  of the address bar, or the floating pill.

When the app is already installed (`display-mode: standalone` matches), the
in-app install prompt and tip stay hidden via `useStandaloneMode()`.

### Service worker + caching

- Generated by `vite-plugin-pwa` (Workbox under the hood). The SW is registered
  on demand via `useRegisterSW` from `virtual:pwa-register/react` — never via
  `navigator.serviceWorker.register` directly.
- **App shell** (HTML, JS, CSS, fonts, icons) is precached on first visit so
  cold starts hit memory rather than the network.
- **Google Fonts** — stylesheets stale-while-revalidate, font files cache-first
  for one year.
- **Supabase Storage** (public bucket reads — banners, logos, avatars) —
  cache-first, ~30 days, 200-entry cap.
- **Supabase REST** (`/rest/v1/...`) — network-first with a 3-second timeout
  and a 5-minute fallback cache. A flaky link still surfaces the last-known
  response instead of locking the user out.
- **Realtime + edge functions + auth** — network-only. Anything that talks
  to live state always hits the wire.
- **Generic images on other CDNs** — cache-first, ~30 days.

### Updates

- `registerType: 'prompt'` — when a new SW is detected, `PWAUpdatePrompt`
  raises a Sonner toast with an **Update** action. Nothing is auto-reloaded.
- The active tab also polls the SW every hour as a soft safety net for
  long-lived sessions.
- `cleanupOutdatedCaches: true` removes precached assets from prior deploys
  so users don't accumulate stale chunks over time.

### Native-feel polish

- `index.html` ships full `apple-mobile-web-app-*` meta + `apple-touch-startup-image`
  splash entries for common iPhone resolutions to prevent the white flash on
  cold launch.
- `src/index.css` defines `pt-safe`, `pb-safe`, `px-safe`, etc. that read
  `env(safe-area-inset-*)` for notched/pill-cutout devices.
- `<DashboardTopBar />` and `<SiteHeader />` apply `pt-safe` so they sit below
  the iOS status bar in standalone mode.
- An `.app-chrome` utility disables text selection and the iOS callout on
  navigation surfaces (top bar, sidebar). Content areas keep selectable text.
- `overscroll-behavior: none` on `<html>` blocks the iOS rubber-band bounce
  on the app shell while preserving overscroll inside scroll containers.
- `-webkit-tap-highlight-color: transparent` removes the gray flash on tap.

### Code paths

- `vite.config.ts` — `VitePWA` plugin config (Workbox runtime caching, manifest reuse, navigation fallback)
- `public/site.webmanifest` — name, theme/background color, icons, shortcuts to `/u/me/events`, `/dashboard/events`, `/discover`
- `src/components/pwa/PWAUpdatePrompt.tsx` — Sonner-driven update flow
- `src/components/pwa/PWAInstallPrompt.tsx` — `beforeinstallprompt` capture + iOS tip
- `src/hooks/useStandaloneMode.ts` — display-mode detection (incl. iOS `navigator.standalone`)
- `index.html` — apple-touch-startup-image entries, `apple-mobile-web-app-status-bar-style="black-translucent"`, `viewport-fit=cover`
- `src/index.css` — safe-area utilities + `.app-chrome` class

### Bypass / debugging

To unregister the SW during development, open DevTools → Application →
Service Workers → Unregister, then reload. The dev server intentionally
leaves the SW disabled (`devOptions.enabled: false`) to avoid HMR cache
conflicts, so you'll only see the SW after `bun run build && bun run preview`.
